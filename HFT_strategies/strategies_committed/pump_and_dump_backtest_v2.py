        ...
        if len(vol_buy_matched) >0:
            ...
            if vol_buy_cancelled_at_t > mean_vol_buy_matched_t * self.params["threshold1"]:
                if len(MD.getPsellVsellMatched(start, end-1)[0]) > 0:
                    ...
                    print(f"price_sell_min_matched_t_add_1: {price_sell_min_matched_t_add_1}")
                    price_bid_max_matched_t_minus_1 = max(MD.getPbidVbidmatched(start, end - 2)[0])
                    print(f"price_bid_max_matched_t_minus_1: {price_bid_max_matched_t_minus_1}")
                    price_bid_min_matched_t_minus_4 = min(MD.getPbidVbidmatched(start, end - 5)[0])
                    print(f"price_bid_min_matched_t_minus_4: {price_bid_min_matched_t_minus_4}")
                    ask_price = df["ask_price"][-1]
                    print(f"Ask_price: {ask_price}")
                    bid_price = df["bid_price"][-1]
...
if __name__ == '__main__':
    # df = pd.read_csv('/home/thanhnhan/Desktop/pthnhan_quant/HFT_strategies/strategies/VN30F1M.csv', index_col="Date",
    #                  parse_dates=['Date'])
    # df = df[df.index.isnull() == False]
    # df_date = df[df.index.strftime("%Y-%m-%d").isin(["2020-09-28"])]
    ...
    # MD = ManipulationDetection(df_date_hour_morning)
    # l = 200
    # while l<len(df_date_hour_morning):
        ...
        while count <3:
            ...
            while l < len(df_date_hour_morning_or_afternoon) - 200:
                ...
                if detect == "pump and dump":
                    ...
                    print(f"s - temp: {s-temp}")
                    if len(dump) == len(pump_and_dump) and l < len(df_date_hour_morning_or_afternoon) - 1000 and (s - temp < 0.5 and s-temp > -0.5):
    ...
    for i in range(29,30):
        print(f"Ngay {i+1}/9/2020: {date[i-29]}")
